...
var DATA_FILE = import_path.default.join(rootDir, "zara_database.json");
var MESSAGES_FILE = import_path.default.join(rootDir, "zara_messages.json");
var SQLITE_DB_FILE = process.env.SQLITE_DB_PATH;
if (!SQLITE_DB_FILE) {
  if (import_fs.default.existsSync("/app_data")) {
    SQLITE_DB_FILE = import_path.default.join("/app_data", "zara_database.sqlite");
  } else {
    SQLITE_DB_FILE = import_path.default.join(rootDir, "zara_database.sqlite");
  }
} else {
  const parentFolder = import_path.default.dirname(SQLITE_DB_FILE);
  try {
    if (!import_fs.default.existsSync(parentFolder)) {
      import_fs.default.mkdirSync(parentFolder, { recursive: true });
    }
  } catch (err) {
    console.error(`Failed to create parent folder for custom SQLite path: ${parentFolder}`, err);
  }
}
async function syncAllStateToIndividualTables(parsed, isMariaDB, poolOrDb) {
  try {
    if (!parsed) return;
    ...
    if (isMariaDB) {
      ...
      if (parsed.products && Array.isArray(parsed.products)) {
        const pIds = parsed.products.map((p) => p.id);
        if (pIds.length > 0) {
          await poolOrDb.query(`DELETE FROM products WHERE id NOT IN (${pIds.map(() => "?").join(",")})`, pIds);
        } else {
          await poolOrDb.query(`DELETE FROM products`);
        }
        const vIds = parsed.products.reduce((acc, p) => acc.concat((p.variants || []).map((v) => v.id || `${p.id}-${v.size}-${v.color}`)), []);
        if (vIds.length > 0) {
      ...
      if (parsed.orders && Array.isArray(parsed.orders)) {
        const oIds = parsed.orders.map((o) => o.id);
        if (oIds.length > 0) {
          await poolOrDb.query(`DELETE FROM orders WHERE id NOT IN (${oIds.map(() => "?").join(",")})`, oIds);
        } else {
          await poolOrDb.query(`DELETE FROM orders`);
        }
        const itemIds = parsed.orders.reduce((acc, o) => acc.concat((o.items || []).map((item) => item.id || `${o.id}-${item.product?.id || ""}-${item.variant?.id || ""}`)), []);
        if (itemIds.length > 0) {
        ...
        const payIds = parsed.orders.reduce((acc, o) => acc.concat((o.payments || []).map((p) => `${o.id}-${p.method}`)), []);
        if (payIds.length > 0) {
      ...
      if (parsed.cashMovements && Array.isArray(parsed.cashMovements)) {
        const ids = parsed.cashMovements.map((m) => m.id);
        if (ids.length > 0) {
      ...
      if (parsed.stockMovements && Array.isArray(parsed.stockMovements)) {
        const ids = parsed.stockMovements.map((m) => m.id);
        if (ids.length > 0) {
      ...
      if (parsed.auditLogs && Array.isArray(parsed.auditLogs)) {
        const ids = parsed.auditLogs.map((l) => l.id);
        if (ids.length > 0) {
      ...
      if (parsed.customers && Array.isArray(parsed.customers)) {
        const ids = parsed.customers.map((c) => c.id);
        if (ids.length > 0) {
      ...
      if (parsed.promotions && Array.isArray(parsed.promotions)) {
        const ids = parsed.promotions.map((p) => p.id);
        if (ids.length > 0) {
      ...
      if (parsed.pendingTickets && Array.isArray(parsed.pendingTickets)) {
        const ids = parsed.pendingTickets.map((t) => t.id);
        if (ids.length > 0) {
      ...
      if (parsed.wholesalers && Array.isArray(parsed.wholesalers)) {
        const ids = parsed.wholesalers.map((w) => w.id);
        if (ids.length > 0) {
      ...
      if (parsed.wholesaleOrders && Array.isArray(parsed.wholesaleOrders)) {
        const ids = parsed.wholesaleOrders.map((o) => o.id);
        if (ids.length > 0) {
      ...
      if (parsed.sessionsHistory && Array.isArray(parsed.sessionsHistory)) {
        const ids = parsed.sessionsHistory.map((s) => s.id);
        if (ids.length > 0) {
      ...
      const runSqliteUpsert = (sql, params) => {
        try {
...
async function startServer() {
  ...
  const sqliteDb = new import_better_sqlite3.default(SQLITE_DB_FILE);
  ...
  if (useMariaDB) {
    console.log("\u{1F504} Attempting structure connection to MariaDB...");
    try {
      mariadbPool = import_promise.default.createPool({
      ...
      console.warn("\u{1F449} MariaDB is not reachable (normal in the Cloud Run sandbox). Falling back to SQLite for preview.");
      useMariaDB = false;
    }
  }
  if (useMariaDB && mariadbPool) {
    try {
      const [rows] = await mariadbPool.query("SELECT data FROM app_state WHERE id = 1");
      const isMariaDbEmpty = !rows || rows.length === 0 || !rows[0].data || rows[0].data === "{}";
      if (isMariaDbEmpty) {
        console.log("\u{1F504} MariaDB is empty. Migrating existing SQLite / JSON snapshot data to MariaDB...");
        let migratedDataStr = "";
        let migratedMessages = [];
        try {
          const sqliteRow = sqliteDb.prepare("SELECT data FROM app_state WHERE id = 1").get();
        ...
        if (!migratedDataStr || migratedDataStr === "{}") {
          try {
            migratedDataStr = await import_promises.default.readFile(DATA_FILE, "utf-8");
          } catch (e) {
            try {
              const backupFile = import_path.default.join(rootDir, "zara_gestion_db", "zara_data.json");
              migratedDataStr = await import_promises.default.readFile(backupFile, "utf-8");
            } catch (backupErr) {
            }
          }
        }
          ...
          try {
            ...
            if (parsed.orders && Array.isArray(parsed.orders)) {
              for (const order of parsed.orders) {
                ...
                const paymentMethodStr = order.payments?.map((p) => p.method).join(", ") || "CASH";
                await mariadbPool.query(
        ...
        try {
          const sqliteMsgs = sqliteDb.prepare("SELECT * FROM app_messages").all();
        ...
        if (migratedMessages.length === 0) {
          try {
            const backupFile = import_path.default.join(rootDir, "zara_gestion_db", "zara_data.json");
            const backupContent = await import_promises.default.readFile(backupFile, "utf-8");
    ...
    let existingStateRow = sqliteDb.prepare("SELECT data FROM app_state WHERE id = 1").get();
    let isDbStateEmpty = !existingStateRow || !existingStateRow.data || existingStateRow.data === "{}" || existingStateRow.data === '{"products":[]}';
    if (isDbStateEmpty) {
      let initialData = "{}";
      try {
        initialData = await import_promises.default.readFile(DATA_FILE, "utf-8");
        console.log("\u2705 Found root backup data from zara_database.json");
      } catch (e) {
        try {
          const backupFile = import_path.default.join(rootDir, "zara_gestion_db", "zara_data.json");
          initialData = await import_promises.default.readFile(backupFile, "utf-8");
    ...
    const msgRow = sqliteDb.prepare("SELECT COUNT(*) as count FROM app_messages").get();
    if (msgRow.count === 0) {
      try {
        let messagesData = "";
        try {
          messagesData = await import_promises.default.readFile(MESSAGES_FILE, "utf-8");
        } catch (e) {
          const backupFile = import_path.default.join(rootDir, "zara_gestion_db", "zara_data.json");
          const backupContent = await import_promises.default.readFile(backupFile, "utf-8");
          const parsed = JSON.parse(backupContent);
  ...
  try {
    ...
    if (useMariaDB && mariadbPool) {
      ...
      const dbRow = sqliteDb.prepare("SELECT data FROM app_state WHERE id = 1").get();
      if (dbRow && dbRow.data) {
  ...
  const uploadsDir = process.env.UPLOADS_DIR ? import_path.default.resolve(process.env.UPLOADS_DIR) : import_path.default.join(rootDir, "uploads");
  try {
  ...
  app.post("/api/data", async (req, res) => {
    try {
      ...
      import_promises.default.writeFile(DATA_FILE, dataStr).catch((err) => console.error("Backup write error", err));
      const backupDirFile = import_path.default.join(rootDir, "zara_gestion_db", "zara_data.json");
      import_promises.default.writeFile(backupDirFile, dataStr).catch((err) => console.error("Backup folder write error", err));
  ...
  app.get("/api/messages", async (req, res) => {
    try {
      let rows = [];
      if (useMariaDB && mariadbPool) {
        const [dbRows] = await mariadbPool.query("SELECT * FROM app_messages ORDER BY timestamp ASC");
        rows = dbRows;
      } else {
        rows = sqliteDb.prepare("SELECT * FROM app_messages ORDER BY timestamp ASC").all();
      }
      const messages = rows.map((r) => ({
        id: r.id,
        senderId: r.sender_id,
        receiverId: r.receiver_id === "GLOBAL" ? void 0 : r.receiver_id,
        text: r.text,
  ...
  app.post("/api/upload", async (req, res) => {
    try {
      ...
      const cleanName = import_path.default.basename(filename).replace(/[^a-zA-Z0-9.\-_]/g, "");
  ...
  app.get("/api/health", (req, res) => {
    res.json({ status: "running", serverTime: (/* @__PURE__ */ new Date()).toISOString(), mode: "local-offline" });
  });
  ...
  app.post("/api/db/import-json", async (req, res) => {
    try {
      if (!import_fs.default.existsSync(DATA_FILE)) {
        return res.status(404).json({ error: `Le fichier de sauvegarde JSON n'a pas \xE9t\xE9 trouv\xE9 sur le serveur \xE0 l'emplacement: ${DATA_FILE}` });
      }
      const rawData = await import_promises.default.readFile(DATA_FILE, "utf-8");
      const parsed = JSON.parse(rawData);
      if (!parsed.products || !Array.isArray(parsed.products)) {
        return res.status(400).json({ error: "Le fichier JSON n'appartient pas \xE0 Zara Gallery ou a un format incorrect." });
      }
      ...
      await import_promises.default.writeFile(DATA_FILE, rawData).catch(() => {
      });
      const backupDirFile = import_path.default.join(rootDir, "zara_gestion_db", "zara_data.json");
      await import_promises.default.writeFile(backupDirFile, rawData).catch(() => {
      });
      ...
      res.json({
        success: true,
        message: "Importation et synchronisation compl\xE8tes effectu\xE9es !",
        stats: {
      ...
      res.status(500).json({ error: "\xC9chec de l'int\xE9gration du fichier de sauvegarde : " + e.message });
    }
  });
...
`;
        sqlDump += `-- Exported on: ${(/* @__PURE__ */ new Date()).toISOString()}
`;
        sqlDump += `-- ====================================================
...
`;
          const [showRows] = await mariadbPool.query(`SHOW CREATE TABLE \`${tableName}\``);
          if (showRows && showRows.length > 0) {
            sqlDump += `${showRows[0]["Create Table"]};

`;
          }
          const [rows] = await mariadbPool.query(`SELECT * FROM \`${tableName}\``);
          if (rows && rows.length > 0) {
...
`;
        ...
        for (const table of tables) {
          ...
          const colInfo = sqliteDb.prepare(`PRAGMA table_info("${tableName}")`).all();
...
`;
          sqlDump += `DROP TABLE IF EXISTS "${tableName}";
`;
          const createInfo = sqliteDb.prepare(`SELECT sql FROM sqlite_master WHERE type='table' AND name=?`).get(tableName);
...
`;
  ...
  const listenWithFallback = (portToTry) => {
    const serverInstance = app.listen(portToTry, "0.0.0.0", () => {
      console.log("----------------------------------------------------");
      console.log(`ZARA GALLERY - SERVEUR LOCAL ACTIV\xC9`);
      console.log(`Acc\xE8s local : http://localhost:${portToTry}`);
      console.log(`Acc\xE8s Boutique (Wi-Fi) : http://[VOTRE-IP]:${portToTry}`);
      console.log("----------------------------------------------------");
    });
    serverInstance.on("error", (err) => {
      const fallbackPort = PORT + 1;
      if (err.code === "EADDRINUSE" && portToTry === PORT) {
        console.warn(`\u26A0\uFE0F Le port ${portToTry} est d\xE9j\xE0 occup\xE9. Tentative de repli automatique sur le port ${fallbackPort}...`);
        listenWithFallback(fallbackPort);
      } else {
        console.error("\u274C Erreur de d\xE9marrage du serveur :", err);
      }
    });
  };
